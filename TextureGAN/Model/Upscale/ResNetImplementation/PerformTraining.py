...
def PerformTraining(config,hyperparameters,tracker):
    ...
    train_generator(tracker,generator,dataloader,optimizer_G,loss=perceptual_loss_fn,device=device, end_epoch=generator_only_epochs)

    # Train the generator, discriminator
    train_loop(tracker,generator,discriminator,dataloader,optimizer_G,optimizer_D,adversarial_loss=adversarial_loss,total_loss=augmented_loss,device=device, end_epoch=epochs)